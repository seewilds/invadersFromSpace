...
class TitleScreen {
  ...
  constructor(context: CanvasRenderingContext2D, renderOptions: RenderOptions) {
    ...
    this.title = new Text(
      ...
      { x: this.centreX("INVADERS FROM SPACE", this.titleScale, 6), y: -100 },
      "INVADERS FROM SPACE",
      "rgba(0, 255, 0, 1)",
      6,
    );
  ...
  centreX(
    ...
    return (
      (this.context.canvas.width - spacingOverride * scale * text.length) / 2
    );
  }
  ...
  updatePressStart(): void {
    ...
    if (this.pressStartFadeCurrent < this.pressStartFadeEnd) {
      ...
      this.pressStart.updateTextPosition(
        0,
        0,
        `rgba(178, 34, 34, ${this.pressStartFadeCurrent})`,
      );
  ...
  setTextToFinal(): void {
    ...
    this.pressStart.updateTextPosition(
      0,
      0,
      `rgba(178, 34, 34, ${this.pressStartFadeEnd})`,
    );
  }
  ...
  fadeOut(secondsElapsed: number): boolean {
    if (this.titleOpacity > 0 || this.pressStartFadeCurrent > 0) {
      ...
      this.pressStartFadeCurrent -= 0.01;
      this.title.updateTextPosition(
        0,
        0,
        `rgba(0, 255, 0, ${this.titleOpacity})`,
      );
      this.pressStart.updateTextPosition(
        0,
        0,
        `rgba(205, 62, 81, ${this.pressStartFadeCurrent})`,
      );
...
class TransitionScreen {
  ...
  constructor(
    ...
    this.mainText = new Text(
      this.context!,
      this.renderOptions.scale,
      { x: this.centreX(mainText, this.renderOptions.scale, 6), y: 200 },
      mainText,
      mainColour,
      6,
    );
    this.subText = new Text(
      this.context!,
      this.renderOptions.scale,
      { x: this.centreX(subText, this.renderOptions.scale, 6), y: 250 },
      subText,
      subColour,
      6,
    );
...
class PlayerSection {
  ...
  clearDefenders() {
    this.defenderLives.forEach((pixels) =>
...
class ScoreBoard {
  ...
  constructor(
    ...
    this.currentScoreText = new Text(
      this.context!,
      2,
      { x: 10, y: 10 },
      `CURRENT SCORE`,
    ...
    this.currentScore = new Text(
      this.context!,
      2,
      { x: 10, y: 40 },
      `${this.currenctScoreNumber.toString()}`,
      "white",
    );
